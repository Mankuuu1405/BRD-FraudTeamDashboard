...
export default function DepartmentManagement() {
    ...
    if (loading) return <div className="p-6 bg-white rounded-xl border border-gray-200 shadow-sm animate-pulse">Loading departments...</div>;
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
            <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gradient-to-r from-gray-50 to-white">
                <div>
                    <h2 className="text-lg font-bold text-gray-800">Departments & Teams</h2>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {groups.map((group) => (
                    <div key={group.id} className="p-4 rounded-xl border border-gray-100 hover:border-indigo-200 hover:shadow-md transition-all group bg-white cursor-pointer overflow-hidden relative">
                        <div className="absolute top-0 right-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity">
                            <HiUserGroup size={48} className="text-indigo-600" />
                        </div>
                        <div className="flex items-center gap-3 mb-2">
                            <div className="w-8 h-8 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600">
                                <HiUserGroup size={18} />
                            </div>
                            <h3 className="font-bold text-gray-800 truncate pr-4">{group.name}</h3>
                        </div>
